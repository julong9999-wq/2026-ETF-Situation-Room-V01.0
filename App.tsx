...
const APP_VERSION = 'V.01.26'; // Internal Logic Version 
const DISPLAY_VERSION = 'V1.20'; // UI Display Version (Fix Build Error)
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div className="text-left">
                                    ...
                                    <div className="text-xs text-emerald-500">連線伺服器比對 metadata.json</div>
                                </div>
                            </div>
                        </button>
      ...
      try {
          ...
          if (response.ok) {
              ...
              if (serverVersion && serverVersion !== DISPLAY_VERSION) {
                  ...
                  if (manual) alert(`偵測到新版本 ${serverVersion}，即將跳出更新視窗。`);
                  return true;
              } else {
                  if (manual) alert(`目前已是最新版本 (${DISPLAY_VERSION})`);
              }
          } else {
  ...
  if (isInitializing) {
      return (
          <div className="flex flex-col items-center justify-center h-screen bg-blue-50 text-blue-900">
              ...
              <h2 className="text-2xl font-bold mb-2">系統載入中 (V1.20)...</h2>
              <div className="bg-white/50 px-6 py-4 rounded-xl text-center border border-blue-200 max-w-sm">
                  <p className="text-sm text-blue-800 font-bold mb-1">正在套用 V1.20 更新</p>
              </div>
          </div>
      );
  }
