              ...
              <div className="flex items-center gap-2 overflow-x-auto no-scrollbar mask-gradient-right">
                ...
                {subOptions.map(sub => (
                      <button key={sub} onClick={() => setSubFilter(sub === '全部' ? 'ALL' : sub)}
                          className={`px-3 py-1.5 rounded-lg text-base whitespace-nowrap transition-colors font-bold border shrink-0 ${(subFilter === sub || (subFilter === 'ALL' && sub === '全部')) ? 'bg-blue-800 text-white border-blue-800 shadow-sm' : 'bg-blue-50 text-blue-600 border-blue-100 hover:bg-blue-100 hover:text-blue-700'}`}>{sub}</button>
                  ))}
              </div>
                    ...
                    <>
                        <div className="p-3 bg-blue-50 border-b border-blue-100 flex flex-wrap gap-4 justify-between items-center flex-none">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setActiveModal('DIV')} className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 text-purple-700 border border-purple-200 rounded-lg hover:bg-purple-100 font-bold text-base whitespace-nowrap shadow-sm transition-colors">
